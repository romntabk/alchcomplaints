...
def draw_chart_number_of_complaints_for_companies(c1_complaints, c2_complaints, company1, company2):
	...
	'''
	fig = go.Figure()
	fig.add_trace(go.Bar(x=[i[0] for i in c1_complaints], y=[i[1] for i in c1_complaints],  name=f'Жалобы для компании {company1}'))
	fig.add_trace(go.Bar(x=[i[0] for i in c2_complaints], y=[i[1] for i in c2_complaints],  name=f'Жалобы для компании {company2}'))
